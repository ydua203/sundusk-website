...
**Build log (kept up to date as work lands):**
- Day 1 — scaffold, design tokens, fonts, Drizzle schema, seed script. See `docs/DAY-1-NOTES.md`.
- Day 2 — layout shell: header, mobile menu, footer, Section/Container primitives. See `docs/DAY-2-NOTES.md`.
- Day 3 — product card/grid, `/shop`, `/collections/tops`, dresses coming-soon. See `docs/DAY-3-NOTES.md`.
- Day 4 — product detail page: gallery, size selector, size-guide link, accordions. See `docs/DAY-4-NOTES.md`.
- Day 5 — cart: context + localStorage, cart page, quantity, remove, totals. See `docs/DAY-5-NOTES.md`.
- Day 6 — checkout form + Zod, `POST /api/checkout`, order + Razorpay order creation. See `docs/DAY-6-NOTES.md`.
- Between days 6 and 7 — promo code (`SUNDUSK@1`, ₹150 off) and a custom-fit request flow (`/custom-fit`) for sizing beyond XL, added on request. Not in the original spec — updated hard rule 1 to allow factual custom-sizing mentions on product pages. See `docs/PROMO-AND-CUSTOM-FIT-NOTES.md`.
- Day 7 — Razorpay Checkout on the client, `/order/[orderNumber]`, webhook route with signature verification and idempotency. A full test-mode payment tested for real, twice (one genuine failure, one genuine success). See `docs/DAY-7-NOTES.md`.
- Day 8 — emails: Resend, React Email templates, confirmation and shipped. A real email was sent and delivered. See `docs/DAY-8-NOTES.md`.
- Between days 8 and 9 — real product lineup (names + model-shoot photos) replaced the 6 Day-1 placeholders; prices and the model note followed shortly after, real and confirmed. A source-photo mirroring issue was found and partly corrected, and a white-background cover photo was added as photo 1 for every product. Fabric/fit/care are the one remaining placeholder. `ProductImagePlaceholder` retired in favor of real `next/image`. See `docs/PRODUCT-PHOTOS-NOTES.md`.
- Day 9 — auth: register, login, `/account`, order history, RLS policies. Found and fixed a real bug (customers row wasn't cascading on auth user deletion — no FK existed despite the schema comment claiming one). See `docs/DAY-9-NOTES.md`.
- Between days 6 and 9 (payments revisited) — a `POST /api/verify-payment` endpoint added alongside the day-7 webhook: verifies the Razorpay client-side HMAC and confirms the payment server-side via the Razorpay API before doing anything, then calls the same shared `fulfillPaidOrder()` the webhook uses. UX accelerator only — the webhook stays the authoritative source of truth; this endpoint never trusts a client-supplied amount.
- Day 10 — admin: order list, order detail, status transitions, courier + tracking, CSV export. Found and fixed a real gap (`/api/admin/*` routes weren't covered by middleware — only `/admin` pages were) before it shipped. See `docs/DAY-10-NOTES.md`.
- Between days 10 and 11 — the real homepage, built a day ahead of its "natural" slot (see the note below) because the day-1 placeholder had started to look unfinished rather than in-progress. Found and fixed a real bug while researching hero copy: the product descriptions (not just the cover photos) for Black Lace Polka Dot Top and White Polka Dot Top were swapped in the live database — see `docs/PRODUCT-PHOTOS-NOTES.md`'s 2026-08-27 update.
- Day 11 — static pages (`/about`, `/size-guide`, `/shipping`, `/returns`, `/faq`, `/track-order`, `/contact`, `/privacy`, `/terms`), SEO metadata, sitemap, robots, OG image, branded 404. `/track-order` is real new functionality (guest order lookup gated by email, closing a gap flagged on day 7), not just copy — see `docs/DAY-11-NOTES.md`.
- Security audit (requested directly, not one of the 12 days) — found and fixed a critical gap: 5 of 8 database tables (`products`, `product_variants`, `order_items`, `customization_requests`, `webhook_events`) had no Row Level Security, meaning the public Supabase anon key had full read **and write** access to them via Supabase's autogenerated REST API, completely bypassing this app's own server code. Verified live against the real project (not assumed), fixed via RLS + migration `0004`. Also fixed a CSV/formula-injection gap in the admin export, added a real CSP + security headers, and confirmed the legal entity name is "Sundusk". See `docs/SECURITY-AUDIT-NOTES.md`.
- Vercel deployment prep — `vercel.json` (pins function region to `bom1`/Mumbai, matching Supabase's region), `engines.node` in `package.json`, full step-by-step guide. See `docs/DEPLOYMENT.md`.
- **Note:** the 12-day plan below never explicitly schedules the real homepage (`/` is listed as a route in section 9, but no day in section 12 says to build it). Originally decided with the user that `/` would stay the day-1 fonts/palette placeholder until day 11, built alongside the other static pages, rather than building a hero twice before real photography landed. Real photography landed days 8–9 as planned, but the user asked for the real homepage explicitly ahead of day 11 once the placeholder had sat there long enough to look unfinished rather than in-progress — see the "Between days 10 and 11" entry above.
...
- Legal entity: **Sundusk**, a sole proprietorship (confirmed 2026-08-30)
...
- Support phone / WhatsApp: +91 87961 02233
...
Questions: hellosundusk.in@gmail.com  ·  WhatsApp +91 87961 02233
```
...
WhatsApp +91 87961 02233, or email hellosundusk.in@gmail.com, and we will
...
· By WhatsApp on +91 87961 02233, or by email to hellosundusk.in@gmail.com
...
QUESTIONS
hellosundusk.in@gmail.com  ·  WhatsApp +91 87961 02233
```
...
Unsure? WhatsApp us on +91 87961 02233.
```
